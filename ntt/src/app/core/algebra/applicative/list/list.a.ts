...
//   protected readonly lft: ListF<T>; // functorial statefull
//   readonly arr: T[];
...
//   pure: (t: T) => IApplicative<T> = (t) => new ListA(t);

//   appl: <S>(fts: IApplicative<(t: T) => S>) => IApplicative<S> = (fts) =>
//     ListA.from((fts).arr.flatMap((unFunc) => this.mt.arr.map(t => unFunc(t))));
...
//    */
//   applZip: <S>(fts: ListA<(t: T) => S>) => ListA<S> = <S>(
//     fts: ListA<(t: T) => S>
//   ) => {
